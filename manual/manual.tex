...
\DeclareAcronym{bspd}{
  short = BSPD,
  long = brake system plausibility device
}
\DeclareAcronym{bms}{
  short = BMS,
...
\todo{Short description of the system's concept.}
\todo{Block diagram of major components.}
\todo{Motor and inverter.}

\begin{table}[H]
  \centering
  \begin{tabularx}{\textwidth}{|X|X|X|X|X|X|X|X|}
    \hline
    \textbf{LVS voltage} & \textbf{Battery configuration} & \textbf{Battery capacity} & \textbf{Nominal TS voltage} & \textbf{Maximum \acs{ts} voltage} & \textbf{Maximum \acs{ts} current} & \textbf{Motor} & \textbf{Peak motor power} \\
    \hline
    \qty{12}{\V} & 14s5p NMC & \qty{22.5}{\Ah} & \qty{302.4}{\V} & \qty{352.8}{\V} & \qty{225}{\A} & Emrax 228 MV & \qty{79.38}{\kW} \\
    \hline
  \end{tabularx}
  \caption{System Specifications}
  \label{tbl:system-specs}
\end{table}

\section{Tractive System Activation}

\section{Measurement Points}

\chapter{Battery Design}
\section{Segment Design}
\section{Cooling}

\chapter{Shutdown Circuit}
\todo{Block diagram of the overall shutdown circuit.}
\todo{Information on AIRs.}

\section{Insulation Monitoring}
\todo{Bender IMD specifications.}

\section{Inertia Switch}
\section{Latching}
\todo{Talk about single-ended shutdown signal from BMS and IMD latching to a board outside the TSAC.}
\todo{Describe reset method.}

\section{Brake Over-Travel Switch}
\section{High Voltage Disconnect}
\section{Interlocks}
\todo{Additional interlocks on HVD, main HV connectors, inverter.}

\chapter{Brake System Plausibility Device}
The \acf{bspd} is a major component of the shutdown circuit and gets its own chapter due its complexity.

\chapter{Precharge and Discharge}
...
\todo{Plots and schematics of each possible scenario.}
...
\chapter{Tractive System Active Light}
\todo{Schematics of LED, logic and HVD boards.}
\todo{Describe UPS power.}
