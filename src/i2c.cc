...
State handle_event(I2C_TypeDef *i2c, StateMachine *sm) {
    ...
    // Slave address match.
    if (current_state == State::Idle && (sr1 & I2C_SR1_ADDR) != 0) {
        // Clear ADDR bit by reading SR2.
    ...
    // Slave receive.
    if (current_state == State::SlaveRx && (sr1 & I2C_SR1_RXNE) != 0) {
        if (!sm->is_full()) {
            sm->next_byte() = i2c->DR;
        }
        if (sm->is_full()) {
    ...
    // Slave transmit.
    if (current_state == State::SlaveTx && (sr1 & I2C_SR1_TXE) != 0) {
        i2c->DR = !sm->is_full() ? sm->next_byte() : 0xff;
        return State::SlaveTx;
    }

    // Slave stop.
    if ((current_state == State::SlaveRx || current_state == State::SlaveTx) && (sr1 & I2C_SR1_STOPF) != 0) {
        // Clear the stop flag by writing to CR1.
        i2c->CR1 |= 0;
        return current_state == State::SlaveRx ? State::SlaveRxFinish : State::Idle;
    }
