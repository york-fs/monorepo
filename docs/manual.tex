...
Whilst voltage on each side could be retrieved from the \ac{bms} and the inverter over \ac{can} bus, the precharge circuit monitors them both independently in order to to make its operation non-reliant on the \ac{can} network (other than the start command, which could be changed if desired), as well as to be in complete control of the sensing period and ensure that sampling of both voltages is performed at the same instant.
